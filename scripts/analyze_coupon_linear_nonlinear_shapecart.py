...
from benchmark_teacher_guided_atomcolor_cached import _protocol_split_indices, _slice_rows
...
CSV_PATH = REPO_ROOT / "datasets" / "coupon_source.csv"
VARIANTS = (
...
def _run_msplit_worker(
    *,
    ...
    cmd = [
        sys.executable,
        str(REPO_ROOT / "scripts" / "run_msplit_cache_worker.py"),
        "--cache-path",
        str(cache_path),
        "--build-dir",
        str(build_dir),
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--results-root", type=Path, default=REPO_ROOT / "results" / "coupon_gap_analysis")
